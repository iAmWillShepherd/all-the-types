...
const command: GluegunCommand = {
  ...
  run: async toolbox => {
    ...
    // totals
    const allBirds = animals.filter((animal: Animal): animal is Bird => {
      if (animal.group === 'bird') {
        return true
      }
    })

    const allMammals = animals.filter((animal: Animal): animal is Mammal => {
    ...
    type NonHumanMammals = Exclude<Mammal, HumanMammalSpecies>

    const mammalCages = _.chunk<NonHumanMammals>(allMammals, 3)
    ...
    const birdsWithWingspans = _.partition(
      allBirds,
      bird => bird.wingspan !== undefined
    )
    const wingspans = _.map(birdsWithWingspans[0], bird => bird.wingspan)
    ...
    const mediumBirds = _.filter(
      allBirds,
      (bird: any) => bird.wingspan < 10 && bird.wingspan >= 5
    )
    const largeBirds = _.filter(
      allBirds,
      (bird: any) => bird.wingspan < 20 && bird.wingspan >= 10
    )
